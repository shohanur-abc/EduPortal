...
export default function Attendance({ eyebrow, title, description, image, highlights, direction = 'ltr' }: IFeatureDetail) {
    return (
        <Section className='grid grid-cols-1 gap-10 @3xl:gap-16 items-center @3xl:grid-cols-2'>
...
const ContentBlock = ({ eyebrow, title, description, highlights }: Omit<IFeatureDetail, 'image' | 'direction'>) => (
    <div className="flex flex-col justify-center space-y-6 @3xl:space-y-8">
        <div className="space-y-4">
            ...
            <h2 className="text-3xl @xl:text-4xl font-extrabold tracking-tight text-foreground leading-[1.1]">
                {title}
            </h2>
...
const HighlightItem = ({ icon: Icon = CheckCircle, text }: IHighlight) => (
    <li className="flex items-start gap-4 group">
        <div className="mt-1 bg-primary/10 p-1.5 rounded-lg group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-300">
