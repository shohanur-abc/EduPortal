...
const Actions = ({ primary, secondary }: { primary: IDemoCTA['primaryCta']; secondary?: IDemoCTA['secondaryCta'] }) => (
    <div className="flex flex-col @sm:flex-row items-center justify-center gap-4">
        ...
        {secondary && (
            <Button size="lg" variant="secondary" className="rounded-full px-8 border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10" href={secondary.href}>{secondary.text}            </Button>
        )}
    </div>
);
